...
export default function AdminDashboard() {
  ...
  const STATUS_LABEL = { received: "New", preparing: "Preparing", ready: "Ready", delivered: "Delivered", tugallangan: "Done" };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors">
      <div className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400">{activeOrders} active orders · {totalOrders} total</p>
          </div>
          <div className="flex gap-2 flex-wrap justify-end">
            ...
            <button onClick={() => navigate("/admin/menu")} className="hidden sm:flex items-center gap-1 px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"><FiList size={14}/> Menu</button>
            <button onClick={() => navigate("/admin/tables")} className="hidden sm:flex items-center gap-1 px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"><FiGrid size={14}/> Tables</button>
            <button onClick={() => navigate("/admin/staff")} className="flex items-center gap-1 px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"><FiUsers size={14}/> Staff</button>
            <button onClick={logout} className="flex items-center gap-1 px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"><FiLogOut size={14}/> Logout</button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: <FiShoppingBag size={22}/>, num: activeOrders, label: "Active Orders", color: "text-blue-500" },
            { icon: <FiTrendingUp size={22}/>, num: formatUZS(totalRevenue), label: "Revenue (done)", color: "text-green-500" },
            { icon: <FiUser size={22}/>, num: waiters.length, label: "Waiters", color: "text-orange-500" },
            { icon: <FiCoffee size={22}/>, num: chefs.length, label: "Chefs", color: "text-purple-500" },
          ].map(s => (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-50 dark:border-gray-800 flex items-center gap-2">
            ...
            <h2 className="font-bold text-gray-900 dark:text-white">Waiter Performance</h2>
          </div>
          {waiterStats.length === 0 ? (
            <p className="px-6 py-8 text-gray-400 text-sm">No waiters yet. <button onClick={() => navigate("/admin/staff")} className="text-orange-500 underline">Add staff</button></p>
          ) : (
              ...
              {waiterStats.map((w, i) => (
                <div key={w.id} className="px-6 py-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">PIN: {w.pin} · {w.active ? <span className="text-green-500">Active</span> : <span className="text-red-400">Inactive</span>}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="font-bold text-gray-900 dark:text-white">{w.orderCount} orders</p>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-50 dark:border-gray-800 flex items-center gap-2">
            ...
            <h2 className="font-bold text-gray-900 dark:text-white">All Orders</h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-800/50">
                    <th className="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Table</th>
                    <th className="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Type</th>
                    <th className="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Waiter</th>
                    <th className="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Items</th>
                    ...
                    <th className="px-5 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Time</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={o.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td className="px-5 py-3 text-gray-500 dark:text-gray-400">
                          ...
                          {items.length > 2 && <span className="text-gray-400"> +{items.length - 2} more</span>}
                        </td>
