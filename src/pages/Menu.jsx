...
export default function Menu() {
  ...
  function handlePlaceOrder(e) {
    ...
    dispatch(placeOrder({ tableNumber, cart: cartItems, notes, total, customer }));
    dispatch(clearCart());
    setShowCustomerForm(false);
    setCustomer({ name: "", phone: "", people: "1" });
    showToast("Buyurtma qabul qilindi! 🎉");
    setTimeout(() => navigate(`/waiter/orders?table=${tableNumber}`), 800);
  }
      ...
      <div className="sticky top-0 z-30 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between mb-3">
            <button onClick={() => navigate("/")} className="text-sm text-gray-500 dark:text-gray-400 hover:text-orange-500 flex items-center gap-1"><FiArrowLeft size={14} /> Stollar</button>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => navigate(`/waiter/orders?table=${tableNumber}`)} className="text-sm text-gray-500 dark:text-gray-400 hover:text-orange-500 flex items-center gap-1"><FiClipboard size={14} /> Buyurtmalarim</button>
            </div>
          </div>
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Gourmet Restaurant</h1>
